/**
 * All model-facing prompt text for piview planning lives here.
 *
 * Three surfaces:
...
export function planningPrompt(state: PlanState): string {
	...
	sections.push(`[PIVIEW PLANNING ACTIVE]

You are in piview's planning workflow. This is separate from Pi's regular plan mode. Your deliverable this turn is a plan, not code changes. The bar: an engineer with no context on this conversation could pick up your plan and execute it without redoing your investigation.
...
- Do not make changes, stage workarounds, or claim changes were made. If the user asks you to implement something, plan the implementation and note that piview planning is active.
...
Call the piview_plan tool whenever you create or revise the plan — it is the single source of truth shown to the user in the piview GUI. Pass both:
...
If the piview_plan tool is unavailable, fall back to ending your reply with a "Plan:" header followed by numbered steps.`);

	if (state.steps.length > 0) {
		sections.push(`Current plan${state.title ? ` — ${state.title}` : ""} (revise via piview_plan rather than starting over, unless the approach itself changed):
...
export function executionPrompt(state: PlanState): string {
	...
	return `[PIVIEW EXECUTION — pre-piview tool access restored]
...
- If a step turns out to be unnecessary or wrong, do not silently skip it: explain why, and either mark it [DONE:n] with that explanation or call piview_plan to revise the remaining steps.
...
export const UPDATE_PLAN_GUIDELINES = [
	"During piview planning, call piview_plan whenever the plan is created or changes — do not only describe the plan in chat.",
